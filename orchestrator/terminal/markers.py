...
def parse_between_markers(output: str, start_marker: str, end_marker: str) -> str | None:
    ...
    for line in output.splitlines():
        ...
        if stripped == start_marker or (
            start_marker in stripped and len(stripped) < len(start_marker) + 5
        ):
            in_section = True
            continue
        ...
        if in_section:
            if stripped == end_marker or (
                end_marker in stripped and len(stripped) < len(end_marker) + 5
            ):
                break
...
def check_yes_no(
    ...
    cmd = MarkerCommand(f"({check_command}) && echo YES || echo NO", prefix=prefix)
