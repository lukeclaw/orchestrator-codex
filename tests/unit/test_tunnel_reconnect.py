...
class TestReconnectTunnelOnly:
    ...
    def test_tunnel_reconnect_success(
        self, mock_sleep, mock_setup_tunnel, mock_create_window, mock_kill_window, mock_check_tunnel, db
    ):
...
class TestReconnectRdevWorkerTunnelOnlyPath:
    ...
    def test_full_reconnect_when_ssh_dead(
        ...
        # Should NOT use tunnel-only path (will fail due to incomplete mocks, but that's ok)
        try:
            reconnect_rdev_worker(db, mock_session, "orchestrator", "test-worker", 8093, "/tmp", mock_repo)
        except Exception:
            pass  # May fail due to incomplete mocks for full reconnect path
